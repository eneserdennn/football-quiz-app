        ...
        <View className="flex items-center">
            ...
            >
                <View className="h-14 bg-white items-center justify-center rounded-full shadow-md">
                    <View className="bg-white h-14 w-14 rounded-full items-center justify-center">
                        {typeof data === "string" ? (<>
                                {data.toString().length > 7 ? (
                                    <Text className="font-light text-xs text-center">{data}</Text>) : (
                    ...
                    {match && (
                        <View
                            className="absolute bottom-[-10px] bg-white rounded-full h-5 w-5 border border-green-700 items-center justify-center">
...
const SelectedPlayerResult: React.FC<SelectedPlayerResultProps> = ({
    ...
    const teamName = selected.playerTeam.name
        .toLocaleLowerCase()
        .replace(" ", "-");
    ...
    useEffect(() => {
        if (data) {
            if (selected.playerTeam.league.name === "LaLiga") {
                ...
                const leagueData = data.find(
                    (league: any) => league.league_name === selected.playerTeam.league.name
                );
    ...
    useEffect(() => {
        if (selected.playerTeam.league.name === player.playerTeam.league.name) {
            dispatch(setPlayerLeague(selected.playerTeam.league.name));
        }
        if (selected.playerTeam.name === player.playerTeam.name) {
            dispatch(setPlayerClub(selected.playerTeam.name));
        }
        ...
        <View className={`flex bg-white  p-2 my-1 shadow-sm rounded-2xl`}>
            ...
            <View className="flex flex-row justify-between">
                ...
                <PlayerInfo
                    ...
                    match={
                        calculateAge(selected.dateOfBirth) ===
                        calculateAge(player.dateOfBirth)
                    }
                    title="Age"
                    data={String(calculateAge(selected.dateOfBirth))}
                    icon={
                        calculateAge(selected.dateOfBirth) ===
                        calculateAge(player.dateOfBirth) ? null : (
                            <FontAwesome
                                name={`arrow-${
                                    calculateAge(selected.dateOfBirth) >
                                    calculateAge(player.dateOfBirth)
            ...
            <View className="flex pt-2 flex-row justify-center items-center space-x-2">
